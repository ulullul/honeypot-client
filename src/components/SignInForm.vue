<template>
  ...
  <div v-else class="form-container">
    <md-content class="md-elevation-3">
      <div class="title">
        <md-icon class="md-size-5x md-primary">account_circle</md-icon>
        <div class="md-title title__text">Sign in</div>
      </div>
    ...
    <div style="margin-top: 20px">
      Still don't have account? <router-link to="/sign-up">Sign up</router-link>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    async handleSubmit() {
      try {
        ...
        const { data } = await this.$api.user.signIn(this.user);
        this.setToken(data.token);
        await this.$router.replace({ name: 'AllTodos' });
      } catch (e) {
        console.log(e);
      }
    },
